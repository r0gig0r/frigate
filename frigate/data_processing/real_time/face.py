...
class FaceRealTimeProcessor(RealTimeProcessorApi):
    ...
    def weighted_average(
        ...
        Args:
            results_list: A list of tuples, where each tuple contains (name, score, face_area, quality_ok).
            max_weight: The maximum weight to apply based on face area.

        Returns:
            A tuple containing the prominent name, its weighted average score,
            label counts, and quality counts, or (None, 0.0, {}, {}) if the list is empty.
        """
        if not results_list:
            return None, 0.0, {}, {}
